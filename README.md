...
Typical `shard/.env` values include:

```env
SHARD_CHAIN_ENV=testnet
JWT_SECRET=...
SERVER_PRIVATE_KEY=...
THIRDWEB_CLIENT_ID=...
```
...
Notes:
- `SHARD_CHAIN_ENV` selects the built-in contract preset: `local`, `testnet`, or `mainnet`
- `DEV` is opt-in. If unset, the shard treats `DEV` as `false`.
- in `DEV=true` with `SHARD_CHAIN_ENV=local`, the local Hardhat manifest is the preferred source for local contract addresses
- preset values can still be overridden explicitly by setting `SKALE_BASE_RPC_URL`, `IDENTITY_REGISTRY_ADDRESS`, or any other `*_ADDRESS` env

/**
 * One-time deploy script for the WoGGuild contract on the BITE v2 Sandbox chain.
 *
 * Usage:
 *   npx tsx shard/src/deployGuild.ts
 *
 * Requires env vars:
...
async function main() {
  ...
  const solPath = resolve(import.meta.dirname, "../../../hardhat/contracts/WoGGuild.sol");
  const source = readFileSync(solPath, "utf-8");

  console.log("Compiling WoGGuild.sol...");

  // 2. Compile with solc
  const input = {
    language: "Solidity",
    sources: {
      "WoGGuild.sol": { content: source },
    },
    settings: {
      outputSelection: {
        "*": {
  ...
  const compiled = output.contracts["WoGGuild.sol"]["WoGGuild"];
  ...
  console.log(`\nWoGGuild deployed at: ${address}`);
  console.log(`\nAdd to your .env:`);
  console.log(`  GUILD_CONTRACT_ADDRESS=${address}`);
}

...
export async function listCharacterProjectionsForTokenIds(tokenIds: string[]): Promise<CharacterProjectionRecord[]> {
  ...
  const { rows } = await postgresQuery<{
    ...
    `
      select
        ...
        updated_at::text as updated_at
      from game.character_projections
      where character_token_id = any($1::text[])
      order by updated_at desc
    `,
    [normalizedTokenIds]
  );
...
export async function listAllCharacterProjections(): Promise<CharacterProjectionRecord[]> {
  const { rows } = await postgresQuery<{
    wallet_address: string;
    normalized_name: string;
    character_name: string;
...
export async function getCharacterProjectionByNormalizedNameGlobal(
  normalizedName: string
): Promise<CharacterProjectionRecord | null> {

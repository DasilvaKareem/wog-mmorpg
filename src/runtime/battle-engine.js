...
function getSpeedMultiplier(statuses) {
    if (statuses.includes("stop"))
        return 999999;
    if (statuses.includes("haste"))
        return 0.5;
    if (statuses.includes("slow"))
        return 1.5;
    return 1.0;
}
function computeTimeCost(combatant, action) {
    if (combatant.statuses.includes("stop"))
        return 999999;
...
function getNextActor(combatants) {
    const alive = combatants.filter((c) => c.alive);
    alive.sort((a, b) => {
        if (a.nextActTime !== b.nextActTime)
            return a.nextActTime - b.nextActTime;
...
function previewTurnOrder(combatants, steps) {
...
function checkPhase(combatants) {
...
function applyStatusTicks(combatant) {
    const records = [];
...
const defendingSet = new Set();
export class BattleEngine {
    state;
    enemyXpValues;
    constructor(battleId, partyCombatants, enemyCombatants, enemyXpValues) {
    ...
    resolveAction(actor, actionDef, action) {
    ...
    resolveAttack(actor, actionDef, action) {
        const target = this.state.combatants.find((c) => c.id === action.targetId && c.alive);
        if (!target) {
            // Auto-target: pick first alive enemy
            const autoTarget = this.state.combatants.find((c) => c.team !== actor.team && c.alive);
            if (!autoTarget) {
                return {
                    turn: 0, actorId: actor.id, actorName: actor.name,
    ...
    resolveDamage(actor, target, actionDef) {
    ...
    calculateRewards() {
        ...
        const shares = partyMembers.map((m) => {
